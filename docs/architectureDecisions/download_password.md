...
- **Tous les messages de l'API passent en anglais** avec ce ticket, pas seulement ceux du téléchargement : messages de validation, erreurs 404 des tags, descriptions OpenAPI. Le code, ses commentaires et le reste des libellés étaient déjà en anglais, la moitié francophone était l'exception. Le front reste libre d'afficher ses propres textes, ces messages s'adressent d'abord à qui consomme l'API. La documentation du projet, elle, demeure en français.
